...
public static class SpriteExploder {
    ...
    public static List<GameObject> GenerateVoronoiPieces(GameObject source, Transform parentTransform, int extraPoints = 0, int subshatterSteps = 0, Material mat = null)
    {
        List<GameObject> pieces = new List<GameObject>();
        ...
        foreach (List<Vector2> region in voronoi.Regions())
        {
            ...
            {
                pieces.Add(generateVoronoiPiece(source, clippedRegion, origVelocity, origScale, origRotation, mat, parentTransform));
            }
        }
        ...
        if (subshatterSteps > 0)
        {
            ...
            foreach (GameObject piece in pieces)
            {
                morePieces.AddRange(SpriteExploder.GenerateVoronoiPieces(piece, parentTransform, extraPoints, subshatterSteps));
                GameObject.DestroyImmediate(piece);
            }
        }
        else
